...
impl Rule for CopyFromPreviousStage {
    ...
    fn check(&self, doc: &Dockerfile) -> Vec<Finding> {
        ...
        for instruction in &doc.instructions {
            ...
            if copy_from_reference_is_unresolved(instruction, &aliases, stage_count) {
                findings.push(diagnostic(
                    "RDL3022",
                    Severity::Warning,
            ...
            match instruction.keyword.as_str() {
                "FROM" => {
                    if let Some(from) = &instruction.from {
                        pip_no_cache_dir =
                            stage_no_cache.get(&from.image).copied().unwrap_or_default();
                        let stage = from.alias.clone().unwrap_or_else(|| from.image.clone());
                        stage_no_cache.insert(stage.clone(), pip_no_cache_dir);
                        current_stage = Some(stage);
                    }
                }
                "ENV"
                    if instruction.env.as_ref().is_some_and(|env| {
                        env.assignments.iter().any(|assignment| {
                            assignment.name == "PIP_NO_CACHE_DIR"
                                && pip_no_cache_dir_truthy(&assignment.value)
                        })
                    }) =>
                {
                    pip_no_cache_dir = true;
                    if let Some(stage) = &current_stage {
                        stage_no_cache.insert(stage.clone(), true);
                    }
                }
                "RUN"
                    if !pip_no_cache_dir
                        && !has_pip_cache_mount(instruction)
                        && pip_install_missing_no_cache_dir(&instruction.args) =>
                {
                    findings.push(diagnostic(
                        "RDL3042",
                        Severity::Warning,
    ...
    fn check(&self, doc: &Dockerfile) -> Vec<Finding> {
        ...
        for instruction in &doc.instructions {
            match instruction.keyword.as_str() {
                "FROM" => {
                    if let Some(from) = &instruction.from {
                        workdir_set = stage_workdir.get(&from.image).copied().unwrap_or_default();
                        let stage = from.alias.clone().unwrap_or_else(|| from.image.clone());
                        stage_workdir.insert(stage.clone(), workdir_set);
                        current_stage = Some(stage);
                    }
                }
                "WORKDIR" => {
            ...
            .filter(|instruction| {
                instruction.label.as_ref().is_some_and(|label| {
                    label
                        .pairs
                        .iter()
                        .any(|pair| !is_valid_docker_label_key(&pair.key))
                })
            })
            .map(|instruction| {
                diagnostic("RDL3048", Severity::Style, "invalid label key", instruction)
            })
            .collect()
    }
}
        ...
        doc.instructions
            ...
            .map(|instruction| {
                diagnostic(
                    "RDL3051",
                    Severity::Warning,
                    "configured label value is empty",
                    instruction,
                )
            })
            .collect()
        ...
        doc.instructions
            ...
            .map(|instruction| {
                diagnostic(
                    "RDL3054",
                    Severity::Warning,
                    "configured SPDX label is not a valid SPDX identifier",
                    instruction,
                )
            })
            .collect()
        ...
        doc.instructions
            ...
            .filter(|instruction| {
                instruction
                    ...
                    .is_some_and(|alias| matches!(alias, "scratch" | "context"))
            })
            .map(|instruction| {
                diagnostic(
                    "RDL3063",
    ...
    detect_command_invocations(shell)
        ...
        .any(|invocation| {
            invocation
                .arguments
                .iter()
                .any(|argument| matches!(argument.as_str(), "update" | "install"))
        })
}
...
fn pip_install_missing_no_cache_dir(shell: &str) -> bool {
    if shell_sets_pip_no_cache_dir(shell) {
        return false;
    }
    ...
    detect_command_invocations(shell)
        .into_iter()
        .any(|invocation| {
            pip_install_arguments(&invocation).is_some_and(pip_args_missing_no_cache_dir)
        })
}

fn shell_sets_pip_no_cache_dir(shell: &str) -> bool {
    shell
        .split_whitespace()
        .filter_map(|token| token.split_once('='))
        .any(|(name, value)| name == "PIP_NO_CACHE_DIR" && pip_no_cache_dir_truthy(value))
}
...
fn pip_args_missing_no_cache_dir(arguments: &[String]) -> bool {
    arguments.iter().any(|argument| argument == "install")
        && !arguments
            .iter()
...
fn env_references_same_statement_variable(
    ...
    env.assignments
        .iter()
        .enumerate()
        .filter(|(_, assignment)| !known_variables.contains(&assignment.name))
        .any(|(index, assignment)| {
            env.assignments
                .iter()
                .enumerate()
                .filter(|(other_index, _)| *other_index != index)
                .any(|(_, other)| shell_value_references_variable(&other.value, &assignment.name))
        })
}
...
fn is_relative_copy_destination(destination: &str) -> bool {
    ...
    !destination.starts_with('/')
        && !destination.starts_with('$')
        && !is_windows_absolute_path(destination)
}
...
fn is_valid_url_label_value(value: &str) -> bool {
    ...
    !scheme.is_empty()
        ...
        && ((rest.starts_with("//") && rest.trim_start_matches('/').contains('.'))
            || (matches!(scheme, "mailto" | "urn") && !rest.is_empty()))
}
...
fn is_valid_spdx_label_value(value: &str) -> bool {
    let value = value.trim_matches(|character| matches!(character, '\'' | '"'));
    spdx::Licensee::parse(value).is_ok()
}
...
fn missing_healthcheck_findings(doc: &Dockerfile) -> Vec<Finding> {
    ...
    let mut bad_stages = BTreeSet::new();

    for (instruction_index, instruction) in doc.instructions.iter().enumerate() {
        if let Some(from) = &instruction.from {
            let stage = HealthcheckStage {
                source: from.image.clone(),
                name: from.alias.clone().unwrap_or_else(|| from.image.clone()),
                instruction_index,
            };

            if stage_inherits_healthcheck(&stage.source, &good_stages) {
                good_stages.insert(stage.clone());
            } else {
                bad_stages.insert(stage.clone());
            }
            current_stage = Some(stage);
            continue;
        }
        ...
        if instruction.healthcheck.is_some() {
            ...
            let now_good = recursively_good_healthcheck_stages(&bad_stages, stage);
            for inherited in &now_good {
                bad_stages.remove(inherited);
                good_stages.insert(inherited.clone());
            }
            bad_stages.remove(stage);
            good_stages.insert(stage.clone());
        }
    }
    ...
    bad_stages
        .into_iter()
        .filter_map(|stage| doc.instructions.get(stage.instruction_index))
        .map(|instruction| {
            diagnostic(
                "RDL3057",
                Severity::Ignore,
...
fn stage_inherits_healthcheck(source: &str, good_stages: &BTreeSet<HealthcheckStage>) -> bool {
    good_stages.iter().any(|stage| stage.name == source)
}

fn recursively_good_healthcheck_stages(
    bad_stages: &BTreeSet<HealthcheckStage>,
    stage: &HealthcheckStage,
) -> BTreeSet<HealthcheckStage> {
    let direct = bad_stages
        .iter()
        .filter(|candidate| candidate.name == stage.source)
        .cloned()
        .collect::<BTreeSet<_>>();

    direct
        .iter()
        .fold(direct.clone(), |mut accumulated, candidate| {
            accumulated.extend(recursively_good_healthcheck_stages(bad_stages, candidate));
            accumulated
        })
}
...
fn missing_required_labels(doc: &Dockerfile, config: &Config) -> Vec<Finding> {
    ...
    let mut stage_labels: BTreeMap<String, BTreeSet<String>> = BTreeMap::new();
    let mut stage_lines: BTreeMap<String, usize> = BTreeMap::new();
    ...
    for instruction in &doc.instructions {
        match instruction.keyword.as_str() {
            "FROM" => {
                if let Some(from) = &instruction.from {
                    let parent_labels = stage_labels.get(&from.image).cloned().unwrap_or_default();
                    let stage = from.alias.clone().unwrap_or_else(|| from.image.clone());
                    stage_labels.insert(stage.clone(), parent_labels);
                    stage_lines.insert(stage.clone(), instruction.line);
                    current_stage = Some(stage);
            ...
            "LABEL" => {
                if let (Some(stage), Some(label)) = (&current_stage, &instruction.label) {
                    let labels = stage_labels.entry(stage.clone()).or_default();
            ...
            "COPY" => {
                ...
                {
                    referenced_stages.insert(source_stage.clone());
                }
            }
            _ => {}
        }
            ...
            invocation
                ...
                .is_some_and(|install_index| {
                    !invocation
                        ...
                        .skip(install_index + 1)
                        .any(|argument| {
                            matches!(argument.as_str(), "-y" | "--yes" | "--assume-yes")
                        })
                })
        })
}
    ...
    detect_command_invocations(shell)
        ...
        .any(|invocation| {
            ...
            invocation
                ...
                .skip(install_index + 1)
                .filter(|argument| !argument.starts_with('-'))
                .filter(|argument| !argument.contains("://") && !argument.starts_with("git+"))
                .any(|package| !npm_package_has_version(package))
        })
}
...
fn apk_add_has_unpinned_packages(shell: &str) -> bool {
    detect_command_invocations(shell)
        ...
        .any(|invocation| {
            let Some(add_index) = invocation
                .arguments
                .iter()
                .position(|argument| argument == "add")
            else {
                return false;
            };
...
fn yum_install_missing_clean_all(shell: &str) -> bool {
    ...
    let installs = invocations.iter().any(|invocation| {
        invocation.command == "yum" && has_argument(&invocation.arguments, "install")
    });
...
fn has_argument(arguments: &[String], value: &str) -> bool {
    ...
    detect_command_invocations(shell)
        ...
        .any(|invocation| {
            invocation
                .arguments
                .iter()
                .any(|argument| matches!(argument.as_str(), "dist-upgrade" | "dup"))
        })
}
