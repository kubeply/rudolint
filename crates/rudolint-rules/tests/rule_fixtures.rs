...
fn snapshots_rdl3011_valid_expose_port_fixture() {
    let source = read_fixture("rules/RDL3011.valid-expose-port/Dockerfile");
    let document = parse_dockerfile(&source).expect("fixture should parse");
    let findings = RuleEngine::new(Profile::Default, Config::default())
        .lint(&document)
        .into_iter()
...
fn snapshots_rdl3012_single_healthcheck_fixture() {
    ...
    insta::assert_json_snapshot!(
        "rdl3012_single_healthcheck_fixture",
        serde_json::to_value(&cases).expect("cases should serialize")
    );
}
...
fn snapshots_rdl3025_json_entrypoints_fixture() {
    ...
    let findings = engine
        ...
        .collect::<Vec<_>>();
    let fixes = engine.fixes(&document);
...
#[test]
fn snapshots_rdl3054_valid_spdx_labels_fixture() {
    let source = read_fixture("rules/RDL3054.valid-spdx-labels/Dockerfile");
    let document = parse_dockerfile(&source).expect("fixture should parse");
    let config = Config {
        label_schema: BTreeMap::from([
            (
    ...
    insta::assert_json_snapshot!(
        "rdl3054_valid_spdx_labels_fixture",
        serde_json::to_value(&findings).expect("findings should serialize")
    );
}
...
#[test]
fn snapshots_rdl4003_single_cmd_fixture() {
    let fixtures = [
        "RDL4003.no-cmd",
        "RDL4003.single-cmd",
